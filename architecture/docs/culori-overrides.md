...
`COLOR_FAMILIES` in `style-dictionary/actions/web/oklch-formulas.ts` has been updated with the v1.8 design parameters for 16 families. The global lightness range is **Lmax = 0.98** and **Lmin = 0.20**.
...
`hexToCustomOklch` now resolves color families through `resolveColorFamily()`:
...
This lets the formula accept the new canonical family names while still supporting legacy Figma collection names:
...
The parabolic chroma formula remains:

```
C(L) = Cmin + (Cmax - Cmin) × (1 - ((L - Lo) / W)²)
```

Lightness is clamped to `[Lmin, Lmax]`. For `L ≥ Lo` the curve uses `Wlight` and `Clight-min`; for `L < Lo` it uses `Wdark` and `Cdark-min`. When `Lo ± W` falls outside `[Lmin, Lmax]`, the formula clamps the result, so the actual chroma floor at the extreme is higher than `Cmin`.
...
`style-dictionary/actions/web/oklch-formulas.test.ts` now validates:

1. The parabolic chroma boundary conditions for all 16 families.
2. `hexToCustomOklch` returns the family hue and formula-computed chroma for every step in `tokens/options.color.web-light.json`.
3. Legacy aliases produce identical output to canonical family names.
...
A design CSV fixture test is also wired in but skipped until `assets/cedar_token_remap_v3 - Color Token Remap.csv` is available.
