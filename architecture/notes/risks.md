...
|---|---|---|---|---|---|
| **SD v5 `$extensions` resolution** | SD v5 resolves any `{ref}` syntax found anywhere in a token, including inside `$extensions`. Custom code expecting reference strings receives hex values instead. | High | High | **H** | Store path strings without braces in `$extensions.cedar`. Documented in ADR-0005 constraints section. |
...
| **`dictionary.tokens` path navigation** | Option tokens are in `dictionary.tokens` but not `dictionary.allTokens` (filtered out). Code that uses `allTokens` cannot find them. | Medium | Medium | **M** | All option token resolution uses `dictionary.tokens` with explicit path navigation. Documented in ADR-0005 constraints section. |
