import{a as m}from"./load-tokens-BPyqOTXw.js";const u={title:"Tokens/Color/Semantic"};function p(e){return()=>{const o=document.createElement("div");return o.style.cssText="min-height:200px;background:#f5f2eb;",o.innerHTML=`
      <div style="padding:40px 32px;font-family:'DM Mono',monospace;font-size:.85rem;color:#736e65">
        Loading token data…
      </div>`,e().then(r=>{o.innerHTML=r}).catch(r=>{const t=r instanceof Error?r.message:String(r);o.innerHTML=`
...
`;function g(...e){return`
    <nav class="breadcrumb">
      ${e.map((o,r)=>{const t=r===e.length-1;return`<span class="bc-segment${t?" bc-current":""}">${o}</span>${t?"":'<span class="bc-sep">/</span>'}`}).join("")}
    </nav>
  `}function s(e,o,r){return`
    <div class="section-header">
      <span class="section-label">${e}</span>
      <span class="section-title">${o}</span>
      <span class="section-count">${r} tokens</span>
    </div>
  `}function n(e,o){return`
    <div class="token-grid">
      ...
      ${o.map(r=>{const t=e.get(r);if(!t)return"";const{hex:a,ref:f}=t;return`
          <div class="trow-chip-wrap">
            <span class="trow-chip" style="background:${a};"></span>
          </div>
          <div class="trow-token">${r}</div>
          <div class="trow-ref">${f}</div>
          <div class="trow-hex">${a.slice(0,7).toUpperCase()}</div>
        `}).join("")}
    </div>
  `}const i={name:"Surface",render:p(async()=>{const e=await m(),o=["color.surface.base","color.surface.raised"];return e.get("color.surface.base")?`
      <style>${v}</style>
      <div class="page">
        ${g("Cedar Tokens","Color","Semantic","Surface")}
        ...
        <div class="demo-card">
          <div class="demo-layer" style="background:${e.get("color.surface.base")?.hex};">
            <span class="demo-layer-label" style="color:${e.get("color.text.base")?.hex};">color.surface.base</span>
            <span class="demo-layer-desc" style="color:${e.get("color.text.subtle")?.hex};">Base — page background, modal backdrop</span>
          </div>
          <div class="demo-divider" style="background:${e.get("color.border.subtle")?.hex};"></div>
          <div class="demo-layer" style="background:${e.get("color.surface.raised")?.hex};">
            <span class="demo-layer-label" style="color:${e.get("color.text.base")?.hex};">color.surface.raised</span>
            <span class="demo-layer-desc" style="color:${e.get("color.text.subtle")?.hex};">Raised — cards, sidebars, dropdowns</span>
          </div>
        </div>
      </div>
    `:"<p>No tokens loaded</p>"})},l={name:"Text",render:p(async()=>{const e=await m(),o=["color.text.base","color.text.subtle","color.text.link","color.text.link-hover"];return`
      <style>${v}</style>
      <div class="page">
        ${g("Cedar Tokens","Color","Semantic","Text")}
        ...
        <div class="text-demo">
          <div class="text-demo-headline" style="color:${e.get("color.text.base")?.hex};">
            Gear up for your next adventure.
          </div>
          <div class="text-demo-body" style="color:${e.get("color.text.subtle")?.hex};">
          ...
          <div>
            <a class="text-demo-link" href="#" style="color:${e.get("color.text.link")?.hex};">View all collections</a>
            <span class="text-demo-hover-note" style="color:${e.get("color.text.link-hover")?.hex};">hover → ${e.get("color.text.link-hover")?.hex.slice(0,7).toUpperCase()}</span>
          </div>
          <div class="text-demo-divider" style="background:${e.get("color.border.base")?.hex};"></div>
          <div style="display:flex;gap:1.5rem;flex-wrap:wrap;">
            ${o.map(r=>`
                <div style="display:flex;align-items:center;gap:0.4rem;">
                  <span style="width:8px;height:8px;border-radius:50%;border:1px solid var(--rule-heavy);flex-shrink:0;background:${e.get(r)?.hex};"></span>
                  <span style="font-family:var(--font-mono);font-size:0.5625rem;color:var(--ink-faint);letter-spacing:0.04em;">${r}</span>
                </div>
              `).join("")}
          </div>
        </div>
      </div>
    `})},d={name:"Border",render:p(async()=>{const e=await m(),o=["color.border.base","color.border.subtle"];return`
      <style>${v}</style>
      <div class="page">
        ${g("Cedar Tokens","Color","Semantic","Border")}
        ...
        <div class="border-demo-grid">
          <div class="border-demo-cell" style="border:1.5px solid ${e.get("color.border.base")?.hex};">
            <div class="border-demo-label">color.border.base</div>
            <div class="border-demo-desc" style="color:${e.get("color.text.subtle")?.hex};">Default — cards, inputs, containers</div>
            <div style="margin-top:0.75rem; height:1px; background:${e.get("color.border.base")?.hex};"></div>
            <div style="font-family:var(--font-mono); font-size:0.5rem; color:var(--ink-faint); letter-spacing:0.06em; margin-top:0.25rem;">${e.get("color.border.base")?.hex.slice(0,7).toUpperCase()}</div>
          </div>
          <div class="border-demo-cell" style="border:1.5px solid ${e.get("color.border.subtle")?.hex};">
            <div class="border-demo-label">color.border.subtle</div>
            <div class="border-demo-desc" style="color:${e.get("color.text.subtle")?.hex};">Subtle — dividers, section separators</div>
            <div style="margin-top:0.75rem; height:1px; background:${e.get("color.border.subtle")?.hex};"></div>
            <div style="font-family:var(--font-mono); font-size:0.5rem; color:var(--ink-faint); letter-spacing:0.06em; margin-top:0.25rem;">${e.get("color.border.subtle")?.hex.slice(0,7).toUpperCase()}</div>
          </div>
        </div>

        <div style="margin-top:1rem; padding:1.5rem; background:${e.get("color.surface.raised")?.hex}; border-radius:4px; border-top:3px solid ${e.get("color.border.base")?.hex};">
          ...
          <div style="font-family:var(--font-sans); font-size:0.8125rem; color:${e.get("color.text.subtle")?.hex}; line-height:1.6;">
            Use <code style="font-family:var(--font-mono); font-size:0.75rem; color:${e.get("color.text.base")?.hex};">border.base</code> for interactive and structural boundaries. Use <code style="font-family:var(--font-mono); font-size:0.75rem; color:${e.get("color.text.base")?.hex};">border.subtle</code> for low-emphasis visual separators that shouldn't compete with content.
          </div>
        </div>
      </div>
    `})},c={name:"All Semantic Tokens",render:p(async()=>{const e=await m(),o=e.size,r=["color.surface.base","color.surface.raised"],t=["color.text.base","color.text.subtle","color.text.link","color.text.link-hover"],a=["color.border.base","color.border.subtle"];return`
      <style>${v}</style>
      <div class="page">
        ${g("Cedar Tokens","Color","Semantic")}
        ...
        <div class="overview-header">
          ...
          <div style="text-align:right;">
            <div style="font-family:var(--font-mono); font-size:4rem; font-weight:800; color:rgba(46,46,43,0.06); line-height:1; letter-spacing:-0.06em;">${o}</div>
        ...
        <div class="section-block" style="position:relative;">
          ...
          <div style="margin-top:1.25rem;">${n(e,r)}</div>
          <div style="margin-top:1.25rem;" class="demo-card">
            <div class="demo-layer" style="background:${e.get("color.surface.base")?.hex};">
              <span class="demo-layer-label" style="color:${e.get("color.text.base")?.hex};">surface.base</span>
              <span class="demo-layer-desc" style="color:${e.get("color.text.subtle")?.hex};">Page background</span>
            </div>
            <div class="demo-divider" style="background:${e.get("color.border.subtle")?.hex};"></div>
            <div class="demo-layer" style="background:${e.get("color.surface.raised")?.hex};">
              <span class="demo-layer-label" style="color:${e.get("color.text.base")?.hex};">surface.raised</span>
              <span class="demo-layer-desc" style="color:${e.get("color.text.subtle")?.hex};">Cards &amp; panels</span>
            </div>
          </div>
        </div>
        ...
        <div class="section-block" style="position:relative;">
          ...
          ${s("Semantic","Text",4)}
          <div style="margin-top:1.25rem;">${n(e,t)}</div>
        </div>

        <div class="section-block" style="position:relative;">
          <div class="deco-index">03</div>
          ${s("Semantic","Border",2)}
          <div style="margin-top:1.25rem;">${n(e,a)}</div>
        </div>
      </div>
...
}`,...c.parameters?.docs?.source}}};const y=["Surface","Text","Border","AllSemantic"];export{c as AllSemantic,d as Border,i as Surface,l as Text,y as __namedExportsOrder,u as default};
