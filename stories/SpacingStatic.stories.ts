...
import { loadStaticSpacingTokens, type StaticSpacingToken } from "./lib/load-tokens.js";
...
function staticTokenTable(tokens: StaticSpacingToken[]): string {
  ...
  const rows = tokens
    .map((tok) => {
      ...
      <div class="srow-decl" title="${tok.path}">${tok.path}</div>
...
export const SwiftFileCompilation: Story = {
  ...
  render: asyncStory(async () => {
    ...
    const swiftLines = tokens
      .map((t) => `    public static let ${toCamelCase(t.name)} = CGFloat(${t.value})`)
      .join("\n");
