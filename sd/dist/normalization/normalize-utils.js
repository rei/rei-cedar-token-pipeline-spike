 ...
 */
export function buildCollectionToSection(parsed) {
    const map = new Map();
 ...
 */
export function clean(node, collectionToSection) {
    const out = {};
    for (const [key, value] of Object.entries(node)) {
        ...
        if (isLeaf(value)) {
            ...
            out[key] = clean(value, collectionToSection);
        }
    }
    return out;
}
 ...
 */
export function nestUnderSections(cleaned, collectionToSection) {
    const out = {};
 ...
 */
function linearRegression(steps) {
...
export function buildSpacingClampData(spacingFiles) {
    ...
    const map = new Map();
    const allKeys = new Set(withBreakpoints.flatMap(({ scale }) => Object.keys(scale)));
    for (const key of allKeys) {
        const steps = withBreakpoints
